...
namespace Nop.Web.Areas.Admin.Controllers
{
    public partial class CustomerController : BaseAdminController
    {
        ...
        //eyal:
        [HttpPost]     
        public virtual IActionResult UpldateCustomerInLine(ICollection<int> customersId, int selectedRoleId, bool sendWelcomeEmail) {
            ...
            foreach (var customerId in customersId)
            {
                ...
                foreach (var roleFromAll in allCustomerRoles)
                    if (selectedRoleId == roleFromAll.Id)
                        customerRoles.Add(roleFromAll); // add the new customer roles


                // Add the old user roles (other wise they will be removed)
                // make sure we are adding again role that this use already have 
                var model = _customerModelFactory.PrepareCustomerModel(null, customer);
                foreach (var existsRoleId in model.SelectedCustomerRoleIds)
                {
                    foreach (var roleFromAll in allCustomerRoles)

                        if (existsRoleId == roleFromAll.Id && roleFromAll.Id != selectedRoleId)                            
                            customerRoles.Add(roleFromAll);
                }

                
                var customerRolesError = ValidateCustomerRoles(customerRoles, _customerService.GetCustomerRoles(customer));
                if (!string.IsNullOrEmpty(customerRolesError))
                {
                    ModelState.AddModelError(string.Empty, customerRolesError);
                    _notificationService.ErrorNotification(customerRolesError);
                    return Json(new { success = false, responseText = customerRolesError });
                }

                // all this shit up to here is to validate this role can be added
                // now add the roles...                
                _customerService.AddCustomerRoleMapping(new CustomerCustomerRoleMapping { CustomerId = customer.Id, CustomerRoleId = selectedRoleId });
