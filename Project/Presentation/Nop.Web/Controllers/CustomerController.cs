...
namespace Nop.Web.Controllers
{
    ...
    public partial class CustomerController : BasePublicController
    {
        ...
        public virtual IActionResult Register()
        {
            ...
            var customerAvatarModel = new CustomerAvatarModel();
            model.CustomerAvatarModel = _customerModelFactory.PrepareCustomerAvatarModel(customerAvatarModel);
        ...
        //available even when navigation is not allowed
        [CheckAccessPublicStore(true)]
        public virtual IActionResult Register(RegisterModel model, string returnUrl, bool captchaValid, IFormCollection form, IFormFile uploadedFile)
        {
            //check whether registration is allowed
            if (_customerSettings.UserRegistrationType == UserRegistrationType.Disabled)
                return RedirectToRoute("RegisterResult", new { resultId = (int)UserRegistrationType.Disabled });
            ...
            {
                isValidPhoto = false;
                ModelState.AddModelError("", "יש להעלות קובץ צילום תעודת זהות!");
            }
            else
            {

                if (uploadedFile.ContentType != "image/jpeg" && uploadedFile.ContentType != "image/gif")
                {
                    isValidPhoto = false;
                    ModelState.AddModelError("", "יש להעלות תמונה מסוג jpeg או gif");
                }

                if (uploadedFile.Length > avatarMaxSize)
                {
                    isValidPhoto = false;
                    ModelState.AddModelError("", "קובץ צילום תעודת זהות צריך להיות קטן מ - " + (avatarMaxSize) + "bytes");
                }
            }
            
            if (!isValidPhoto)
            {
                var customerAttributesXml2 = ParseCustomCustomerAttributes(form);
                model = _customerModelFactory.PrepareRegisterModel(model, true, customerAttributesXml2);
                return View(model);
            }
            ...
            if (ModelState.IsValid)
            {
                ...
                if (registrationResult.Success)
                {
                    ...
                    //Eyal (insert Avatar on registration:
                    UploadAvatarFromRegistration(uploadedFile);
                    //end insert avatar
        ...
        private void UploadAvatarFromRegistration(IFormFile uploadedFile)
        {
            CustomerAvatarModel model = new CustomerAvatarModel();
            var customer = _workContext.CurrentCustomer;
            try
            {
